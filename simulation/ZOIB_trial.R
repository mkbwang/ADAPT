...
ZOIB_test <- function(mydf){
  ...
  mydf_dp$totcounts <- mydf_dp$T1 + mydf_dp$T2
  mydf_dp$weights <- mydf_dp$totcounts/sum(mydf_dp$totcounts) * nrow(mydf_dp)
  if (nrow(mydf_dp) > 2){
    invisible(beta_model <- betareg(prop ~ covar | covar, weights=weights,  data=mydf_dp) %>% summary())
