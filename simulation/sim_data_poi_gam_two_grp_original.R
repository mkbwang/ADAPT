...
abn.tab.gen1=function(n.taxa, n.samp.grp1, n.samp.grp2, low.abn, med.abn, high.abn,
  ...
  if(balanced.micro.load==TRUE){
    ...
    effect.size[diff.pos]=sample(c(2,5,8,10), size=length(diff.pos), replace=TRUE)
    ...
    # Effect size
    effect.size=rep(1, n.taxa)
    effect.size[diff1.ind]=sample(c(2,5,8,10), size=length(diff1.ind), replace=TRUE)
    effect.size[diff2.ind]=sample(c(1/2, 1/5, 1/8, 1/10), size=length(diff2.ind), replace=TRUE)
