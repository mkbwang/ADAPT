...
# POLDA analysis
polda_result <- polda(otu_table = baxter_count_mat,
...
# separated steps
baxter_glm_alltaxa <- relabd_GLM(baxter_count_mat,
                                 baxter_metadata,
                                 covar="crc")

baxter_reftaxa_count <- baxter_count_mat[polda_result$Reference_Taxa, ]
baxter_glm_reftaxa <- relabd_GLM(baxter_reftaxa_count,
                                 baxter_metadata,
                                 covar="crc")


# plot estimated tau hats for all the taxa
tau_hats <- baxter_glm_alltaxa$effect
median_tau_hat <- median(tau_hats)
Taxa_info_df <- data.frame(Taxa = baxter_glm_alltaxa$Taxa,
                           Tau = tau_hats)
...
baxter_taxonomy_df[polda_df$Taxon, 'polda_effect'] <- polda_df$effect
baxter_taxonomy_df$polda_rawp <- NA
baxter_taxonomy_df[polda_df$Taxon, 'polda_rawp'] <- polda_df$pval
baxter_taxonomy_df$polda_adjustedp <- NA
baxter_taxonomy_df[polda_df$Taxon, 'polda_adjustedp'] <- polda_df$pval_adjust
